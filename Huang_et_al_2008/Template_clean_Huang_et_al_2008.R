...
lat  <- 32.16894981723592
lon  <- 112.34695934083425
srid <- 4326
...
datasets <- list(name        = "Huang_et_al_2008",
                 date        = "2003-06-01",
                 description = "Food web structure of a benthic macroinvertebrate community in a second order stream, Hubei Province, China",
                 public      = TRUE)
...
networks <- list(name             = "Huang_et_al_2008",
                 date             = "2003-06-01",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Huang_et_al_2008'
food_web_name <- 'WEB332'
#Read the files
...
name.of.web[c(2,3,6),2] <- c("Diatoms", "Plantae", "Animalia")
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Diatoms' | sp_taxon_1 == "Fungi" | sp_taxon_1 == "Filament Algae", 'herbivory', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Amorphus Detritus' | sp_taxon_1 == "animal material" |
                                                                 sp_taxon_1 == "Vascular Plant Detritus", 'commensalism', type))
