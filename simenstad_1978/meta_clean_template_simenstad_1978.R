...
folder_name <- "simenstad_1978" # Name of the subfolder in mangal-datasets
food_web_name <- c("WEB48", "WEB49")
...
ref <- list(doi       = "10.1126/science.200.4340.403",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://doi.org/10.1126%2Fscience.200.4340.403",
             data_url  = "https://globalwebdb.com/",
             author    = "C. A. Simenstad",
             year      = "1978",
             bibtex    = "@article{Simenstad_1978, doi = {10.1126/science.200.4340.403}, url = {https://doi.org/10.1126%2Fscience.200.4340.403}, year = 1978, month = {apr}, publisher = {American Association for the Advancement of Science ({AAAS})}, volume = {200}, number = {4340}, pages = {403--411}, author = {C. A. Simenstad and J. A. Estes and K. W. Kenyon}, title = {Aleuts, Sea Otters, and Alternate Stable-State Communities}, journal = {Science}}")
...
dataset <- list(name        = "simenstad_1978",
                 date        = "1969-01-01",
                 description = "Food web of the western Aleutian Islands",
                 public      = TRUE)
...
network <- list(name        = "simenstad_1978",
                 date        = "1969-01-01",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
#   map(~as.factor(.x)) %>%
#   map(~fct_inorder(.x)) %>%
#   set_names(c("Sea otter dominated", "Sea urchin dominated"))
# 
...
#   map(~mutate(.x, sp_id = str_replace(.x$sp_id, fixed("48"), NA_character_))) %>%
#   map(~mutate(.x, sp_id = str_replace(.x$sp_id, fixed("49"), NA_character_))) %>%
#   set_names(c("Sea otter dominated", "Sea urchin dominated"))
...
data_matrice <- paste0("~/Documents/UBO/Cours/Semestre 8/Stage/Mangal/Trophic-metacommunities-master/Trophic_metacom_meta_analysis/interaction matrices/",
  ...
  set_names("Sea otter dominated", "Sea urchin dominated")
...
# 
# taxa$`Sea otter dominated`$letter <- rep("A", times = length(taxa$`Sea otter dominated`$id))
# taxa$`Sea urchin dominated`$letter <- rep("B", times = length(taxa$`Sea urchin dominated`$id))
# 
# taxa <- taxa %>%
...
#   map(~mutate(.x, sp = str_replace_all(.x$sp, c("decapods" = "Decapoda", "sea urchins" = "Echinoidea", "harbor seals" = "Phoca vitulina",
#                                                 "sea otters" = "Enhydra lutris","asteroids" = "Asteroidea", 
#                                                 "prehistoric Aleut man" = "Homo sapiens"))))
# 
...
#   map(~mutate(.x, name_clear = str_replace_all(.x$name_clear, c("Chitons" = "Chitons, gastropods, limpets"))))
