...
annual_rain <- list(name        = "annual rainfall",
              table_owner = "environments",
              description = "The annual rainfall of the study area",
              unit        = "mm")

mean_temp <- list(name        = "mean air temperature",
              table_owner = "environments",
              description = "The mean air temperature of the study area",
              unit        = "°C")

lake_surface <- list(name        = "Lake's surface area",
              table_owner = "environments",
              description = "The surface area of the lake",
              unit        = "km²")

max_depth <- list(name        = "maximum depth",
              table_owner = "environments",
              description = "The maximum depth of the lake",
              unit        = "m")

mean_depth <- list(name        = "mean depth",
              table_owner = "environments",
              description = "The mean depth of the lake",
              unit        = "m")

refs <- list(doi       = "10.1016/j.ecolmodel.2010.09.038",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://www.sciencedirect.com/science/article/pii/S0304380010005259",
            data_url  = "https://globalwebdb.com/",
            author    = "Fetahi",
            year      = "2011",
            bibtex    = "@article{Fetahi_2011, doi = {10.1016/j.ecolmodel.2010.09.038}, url = {https://doi.org/10.1016%2Fj.ecolmodel.2010.09.038}, year = 2011, month = {feb}, publisher = {Elsevier {BV}}, volume = {222}, number = {3}, pages = {804--813}, author = {Tadesse Fetahi and Michael Schagerl and Seyoum Mengistou and Simone Libralato}, title = {Food web structure and trophic interactions of the tropical highland lake Hayq, Ethiopia}, journal = {Ecological Modelling}}")
...
#enviro_data <- read.csv(file=paste0('mangal-datasets-ben/DBase_de_Patrick/',foldername,'/raw/enviro.csv'), header= F, sep=';', stringsAsFactors = F)

enviro_rain <- list(name  = 'annual rainfall',
               lat   = lat,
               lon   = lon,
               srid  = srid,
               date  = "2007-10-01",
               value = 1173)
enviro_temp <- list(name  = 'mean air temperature',
               lat   = lat,
               lon   = lon,
               srid  = srid,
               date  = "2007-10-01",
               value = 18.2)
enviro_lake <- list(name  = "Lake's surface area",
                    lat   = lat,
                    lon   = lon,
                    srid  = srid,
                    date  = "2007-10-01",
                    value = 23.2)
enviro_maxdepth <- list(name  = 'maximum depth',
                    lat   = lat,
                    lon   = lon,
                    srid  = srid,
                    date  = "2007-10-01",
                    value = 88)
enviro_meandepth <- list(name  = 'mean depth',
                    lat   = lat,
                    lon   = lon,
                    srid  = srid,
                    date  = "2007-10-01",
...
datasets <- list(name        = "Fetahi_et_al_2011",
                date        = "2007-10-01",
                description = "Food web structure and diet composition based on an Ecopath model in Lake Hayq, Ethiopia",
                public      = TRUE)
...
networks <- list(name             = "Fetahi_et_al_2011",
                date             = "2007-10-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
inter <- list(date          = "2007-10-01",
              direction     = "directed",
              method        = "Laboratory observation, biblio and model",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
library(reshape2)
library(tibble)
foldername <- 'Fetahi_et_al_2011'
food_web_name <- 'WEB289'
#Read the files
...
name.of.web[3,2] <- "Garra dembecha"
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Phytoplankton', 'herbivory', 'predation'))
...
taxons_df[15,2] <- "Clarias garapienus"
taxons_df[1,2] <- "Oreochromis niloticus"
taxons_df[2,2] <- "Cyprinus carpio"
taxons_df[3,2] <- "Garra dembecha"
taxons_df[5,2] <- "Mesocyclops aequatorialis"
taxons_df[6,2] <- "Thermocyclops ethiopiansis"
...
POST_attribute(attr = annual_rain)
POST_attribute(attr = mean_temp)
POST_attribute(attr = lake_surface)
POST_attribute(attr = max_depth)
POST_attribute(attr = mean_depth)
POST_ref(ref = refs)
