...
folder_name <- "bradstreet_1982" # Name of the subfolder in mangal-datasets
food_web_name <- "WEB87"
...
ref <- list(doi       = "10.14430/arctic2303",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://doi.org/10.14430%2Farctic2303",
             data_url  = "https://globalwebdb.com/",
             author    = "M. Bradstreet",
             year      = "1982",
             bibtex    = "@article{Bradstreet_1982, doi = {10.14430/arctic2303}, url = {https://doi.org/10.14430%2Farctic2303}, year = 1982, month = {jan}, publisher = {The Arctic Institute of North America}, volume = {35}, number = {1}, author = {Michael S.W. Bradstreet and William E. Cross}, title = {Trophic Relationships at High Arctic Ice Edges}, journal = {{ARCTIC}}}")
...
dataset <- list(name        = "bradstreet_1982",
                 date        = "1978-01-01",
                 description = "Food web of High Arctic Ice Edges",
                 public      = TRUE)
...
# sp_name <- sp_name %>% # Remove junk from sp names
#   str_remove_all("\\s\\(.*\\)$") %>% # Correct vernacular name at the lowest level possible (species)
#   str_replace_all(., c("diatoms" = "Bacillariophyta", "arctic cod" = "Arctogadus glacialis", "ringed seals" = "Pusa hispida",
#                        "narwhals" = "Monodon monoceros", "kittiwakes" = "Rissa sp.", "fulmars" = "Fulmarus sp.")) # Modifying names based on NCBI and complementary informations in the source

# sp_name_cleaned <- gnr_resolve(sp_name, canonical = FALSE, best_match_only = T) # Check for wrong names
# sp_name_not_known <- attributes(sp_name_cleaned)$not_known %>% # Names not recognized by Global Names Recognition and Discovery.
...
#          score = NA_real_)
# 
# sp_name <- sp_name_cleaned %>%
#   bind_rows(sp_name_not_known) %>% # Bind rows of sp_name_cleaned and sp_name_not_known
#   select(user_supplied_name, matched_name) %>% # Drop unused variable
#   {left_join(enframe(sp_name, name = NULL, value = "user_supplied_name"), ., by = c("user_supplied_name"))} # Merge sp_name and sp_name_cleaned keeping the order of original sp_name (NB : same as the original file)
# 
# sp_name$matched_name[2] <- "Marine mammal carcasses" # Manually editing some wrong name added by Global Names Recognition and Discovery.
# 
# sp_name <- sp_name %>%
...
#   map(~mutate(.x, sp = str_replace_all(.x$sp, c("diatoms" = "Bacillariophyta", "arctic cod" = "Arctogadus glacialis", "ringed seals" = "Pusa hispida",
#                                                 "narwhals" = "Monodon monoceros", "kittiwakes" = "Rissa sp.", "fulmars" = "Fulmarus sp."))))
# 
...
# 
# taxa_df[[1]]$`name_clear`[4] <- "Marine mammal carcasses" # Manually editing some wrong name added by Global Names Recognition and Discovery.
# 
