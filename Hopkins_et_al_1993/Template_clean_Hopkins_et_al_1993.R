...
refs <- list(doi       = "10.1007/bf01681980",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://www.researchgate.net/profile/Joseph_Torres2/publication/225987587_Trophic_structure_in_open_waters_of_the_marginal_ice_zone_in_the_Scotia-Weddell_confluence_region_during_spring_1983/links/0deec5374ccf458909000000/Trophic-structure-in-open-waters-of-the-marginal-ice-zone-in-the-Scotia-Weddell-confluence-region-during-spring-1983.pdf",
            data_url  = "https://globalwebdb.com/",
            author    = "Hopkins",
            year      = "1993",
            bibtex    = "@article{Hopkins_1993,	doi = {10.1007/bf01681980},	url = {https://doi.org/10.1007%2Fbf01681980},	year = 1993,	month = {jun},	publisher = {Springer Nature},	volume = {13},	number = {6},	pages = {389--397},	author = {Thomas L. Hopkins and David G. Ainley and Jos{\'{e}} J. Torres and Thomas M. Lancraft},	title = {Trophic structure in open waters of the marginal ice zone in the Scotia-Weddell confluence region during spring (1983)},	journal = {Polar Biology}}")
...
datasets <- list(name        = "Hopkins_et_al_1993",
                date        = "1983-01-01",
                description = "Food web structure in the southern Scotia Sea",
                public      = TRUE)
...
networks <- list(name             = "Hopkins_et_al_1993",
                date             = "1983-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Hopkins_et_al_1993'
food_web_name <- 'WEB342'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(str_detect(matrice.interaction$sp_taxon_1, 'Euphausiid molt debris'), 'commensalism', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Diatoms' | sp_taxon_1 == 'Dinoflagellates' | 
                                                                 sp_taxon_1 == 'Silicoflagellates', 'herbivory', type))
