...
refs <- list(doi      = "10.1016/s0140-1963(18)31510-6",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://www.sciencedirect.com/science/article/abs/pii/S0140196318315106",
            data_url  = "http://digitalcommons.rockefeller.edu/context/cohen_joel_laboratory/article/1000/type/native/viewcontent",
            author    = "Schoenly and Reid",
            year      = "1983",
            bibtex    = "@article{Schoenly_1983, doi = {10.1016/s0140-1963(18)31510-6}, url = {https://doi.org/10.1016%2Fs0140-1963%2818%2931510-6}, year = 1983, month = {sep}, publisher = {Elsevier {BV}}, volume = {6}, number = {3}, pages = {253--263}, author = {Kenneth Schoenly and William Reid}, title = {Community structure of carrion arthropods in the Chihuahuan Desert}, journal = {Journal of Arid Environments}}")
...
datasets <- list(name        = "Schoenly_Reid_1983",
                date        = "1980-05-01",
                description = "Food web structure of carrions arthropods, Chihuahan Desert, El Paso County, Texas",
                public      = TRUE)
...
inter <- list(date          = "1980-05-01",
              direction     = "directed",
              method        = "Field observation and biblio",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
folder.name <- 'Schoenly_Reid_1983'
...
names(matrices.interaction) <- c('WEB195', 'WEB196')
...
names.of.web <- list(subset(name.dictionary, name.dictionary$web == 'WEB195.csv'), subset(name.dictionary, name.dictionary$web == 'WEB196.csv'))
names(names.of.web) <- c('WEB195', 'WEB196')
...
names.of.web <- map(names.of.web, ~mutate(.x, scientific_name = ifelse(original_name == "carrion", "Leporidae", scientific_name)))
...
matrices.interaction <- map(matrices.interaction, ~mutate(.x, type = ifelse(sp_taxon_1 == 'rabbit carcass (L. californicus)' | sp_taxon_1 == 'carrion', 'scavenger', 'predation')))
...
#------------------------------
meta <- c('after 4 days', 'after 14 days')
for (i in 1:2) {
  
  
  networks <- list(name             = paste0("Schoenly_Reid_1983_",i),
                   date             = "1980-05-01",
                   lat              = lat,
                   lon              = lon,
                   srid             = srid,
                   description      = paste0("Food web structure of a rabbit carrion after ",meta[i],", Chihuahan Desert, El Paso County, Texas"),
                   public           = TRUE,
