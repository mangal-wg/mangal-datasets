...
refs <- list(doi       = "10.1111/j.1442-9993.2005.01424.x",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://onlinelibrary.wiley.com/doi/full/10.1111/j.1442-9993.2005.01424.x",
            data_url  = "https://globalwebdb.com/",
            author    = "Motta",
            year      = "2005",
            bibtex    = "@article{Motta_2005,	doi = {10.1111/j.1442-9993.2005.01424.x}, url = {https://doi.org/10.1111%2Fj.1442-9993.2005.01424.x}, year = 2005, month = {feb}, publisher = {Wiley}, volume = {30}, number = {1}, pages = {58--73}, author = {Rosines L. Motta and Virginia S. Uieda}, title = {Food web structure in a tropical stream ecosystem}, journal = {Austral Ecology}}")
...
datasets <- list(name        = "Motta_Uieda_2005",
                date        = "1997-08-01",
                description = "Food web structure of a tropical stream (Potreirinho Creek), Itatinga district, south-eastern Brazil",
                public      = TRUE)
...
networks <- list(name             = "Motta_Uieda_2005",
                date             = "1997-08-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
inter <- list(date          = "1997-08-01",
              direction     = "directed",
              method        = "Gut content and biblio",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
library(reshape2)
library(tibble)
foldername <- 'Motta_Uieda_2005'
food_web_name <- 'WEB347'
#Read the files
...
name.of.web[3,2] <- "Detritus"
name.of.web[4,2] <- "Plantae"
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Unicellular algae' | sp_taxon_1 == "Filamentous algae" |
                                                                 sp_taxon_1 == "Vascular hydrophytes" , 'herbivory', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Organic matter', 'commensalism', type))
...
taxons_df[c(41,61),2] <- c("Polypedilum","Polypedilum")
#Taxa_back
...
taxa_back_df[8,4] <- "43966"
taxa_back_df[10,4] <- "43976"
taxa_back_df[11,4] <- "43970"
taxa_back_df[15,4] <- "44024"
taxa_back_df[28,4] <- "129735"
taxa_back_df[47,4] <- "129229"
taxa_back_df[48,4] <- "129842"
taxa_back_df[51,4] <- "129884"
