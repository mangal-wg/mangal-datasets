...
folder_name <- "petipa_1970" # Name of the subfolder in mangal-datasets
food_web_name <- c("WEB31", "WEB32") # Name of the dataset in Trophic metacommunities-master/Trophic_metacom_meta_analysis/Data
...
ref <- list(doi       = "NA",
            ...
            author    = "T. S. Petipa",
            year      = "1970",
            bibtex    = "@incollection{petipa_1970, title = {The food web structure, utilization transport of energy by trophic levels in the planktonic communities}, booktitle = {Marine Food Chains},  author = {T. S. Petipa, E. V. Pavlova, G. N. Mironov}, editor = {J. H. Steele}, pages = {142-167, from p. 154}, year = {1970}, publisher = {(Oliver and Boyd, Edinburgh, 1970)}}")
...
dataset <- list(name        = "petipa_1970",
                date        = "01-01-1970",
                description = "Food web of the plankton communities of the Black Sea",
                public      = TRUE)
...
network <- list(list(name        = "petipa_1970_epiplankton_communities",
                     date        = "01-01-1970",
                     lat              = lat,
                     lon              = lon,
                     srid             = srid,
                     description      = "Food web of the epiplankton communities of the Black Sea",
                     public           = TRUE,
                     all_interactions = FALSE),
                list(name        = "petipa_1970_Bathyplankton_communities",
                     date        = "dd-mm-yy",
                     lat              = lat,
                     lon              = lon,
                     srid             = srid,
                     description      = "Food web of the bathyplankton communities of the Black Sea",
                     public           = TRUE,
                     all_interactions = FALSE))
...
data_matrice <- paste0("~/Documents/UBO/Cours/Semestre 8/Stage/Mangal/Trophic-metacommunities-master/Trophic_metacom_meta_analysis/interaction matrices/",
  ...
  set_names("Epiplankton communities", "Bathyplankton communities")
...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_1, "(.*[Dd]etritu.*)|(.*[Dd]etritu.*(?!.*))|(DOM)|(Discard)|([Dd]issolved.*organic.*)")
                                ,"commensalism", .x$type))) %>%
