...
lat  <- 42.331571652621165
lon  <- -8.616489227229522
srid <- 4326
...
refs <- list(doi       = "10.1016/j.csr.2010.10.010",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://www.sciencedirect.com/science/article/pii/S0278434310003274",
             data_url  = "https://globalwebdb.com/",
             author    = "Filgueira",
             year      = "2011",
             bibtex    = "@article{Filgueira_2011, doi = {10.1016/j.csr.2010.10.010}, url = {https://doi.org/10.1016%2Fj.csr.2010.10.010}, year = 2011, month = {apr}, publisher = {Elsevier {BV}}, volume = {31}, number = {5}, pages = {476--487}, author = {R. Filgueira and B.G. Castro}, title = {Study of the trophic web of San Simon Bay (Ria de Vigo) by using stable isotopes}, journal = {Continental Shelf Research}}")
...
datasets <- list(name        = "Filgueira_Castro_2011",
                 date        = "2000-02-01",
                 description = "Food web structure of an intertidal zone in San Simon Bay, Spain",
                 public      = TRUE)
...
networks <- list(name             = "Filgueira_Castro_2011",
                 date             = "2000-02-01",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
inter <- list(date          = "2000-02-01",
              direction     = "directed",
              method        = "Stable isotopes ratio",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
library(reshape2)
library(tibble)
foldername <- 'Filgueira_Castro_2011'
food_web_name <- 'WEB270'
#Read the files
...
name.of.web <- mutate(name.of.web, scientific_name = ifelse(original_name == "Marine POM I" | original_name == "Marine POM II" |
                                                              original_name == "Marine POM III", "Marine POM", scientific_name))
#Edge format
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Microphytobenthos' | sp_taxon_1 == "Ascophyllum nodosum" | 
                                                                 sp_taxon_1 == "Enteromorpha intestinalis" | sp_taxon_1 == "Fucus vesiculosus" | 
                                                                 sp_taxon_1 == "Ulva spp." | sp_taxon_1 == "Zostera marina", 'herbivory', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Marine POM I' | sp_taxon_1 == "Marine POM II" | 
                                                                 sp_taxon_1 == "Marine POM III" | sp_taxon_1 == "Sediment", 'commensalism', type))
