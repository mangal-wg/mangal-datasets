...
folder_name <- "day_1967" # Name of the subfolder in mangal-datasets
food_web_name <- c("WEB2") # Name of the dataset in Trophic metacommunities-master/Trophic_metacom_meta_analysis/Data
...
ref <- list(doi       = "NA",
            ...
            author    = "J. H. Day",
            year      = "1967",
            bibtex    = "@incollection{day_1967, title = {The biology of Knysna estuary, South Africa}, booktitle = {Estuaries}, author = {J. H. Day}, editor = {G., H. Lauff}, pages = {397-407, from p. 406}, year = {1967}, publisher = {(American Association for the Advancement of Science Publication 83, Washington, DC, 1967)}}")
...
dataset <- list(name        = "day_1967",
                date        = "01-01-1967",
                description = "Food web of Knysna estuary",
                public      = TRUE)
...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_1, "(.*[Dd]etritu.*(?!.*))|(DOM)|(Discard)|([Dd]issolved.*organic.*)"),"commensalism", .x$type))) %>%
