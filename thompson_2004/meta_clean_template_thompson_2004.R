...
folder_name <- "thompson_2004" # Name of the subfolder in mangal-datasets
food_web_name <- c("WEB248") # Name of the dataset in Trophic metacommunities-master/Trophic_metacom_meta_analysis/Data
...
ref <- list(doi       = "10.1111/j.1365-2656.2004.00899.x",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://doi.org/10.1111%2Fj.1365-2656.2004.00899.x",
            data_url  = "https://globalwebdb.com/",
            author    = "ROSS M. THOMPSON",
            year      = "2004",
            bibtex    = "@article{THOMPSON_2004,doi = {10.1111/j.1365-2656.2004.00899.x},url = {https://doi.org/10.1111%2Fj.1365-2656.2004.00899.x},year = 2004,month = {dec},publisher = {Wiley},volume = {74},number = {1},pages = {77--85},author = {ROSS M. THOMPSON and KIM N. MOURITSEN and ROBERT POULIN},title = {Importance of parasites and their life cycle characteristics in determining the structure of a large marine food web},journal = {Journal of Animal Ecology}}")
...
dataset <- list(name        = "thompson_2004",
                date        = "2004-01-01",
                description = "Food web of the intertidal mudflat of Company Bay",
                public      = TRUE)
...
network <- list(name        = "thompson_2004",
                date        = "2004-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_2, "(.*[Dd]etritu.*)|(.*[Dd]etritu.*(?!.*))|(DOM)|(Discard)|([Dd]issolved.*organic.*)|(.*[Ff]ecal.*)")
                                ,"commensalism", .x$type))) %>%
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_2, "(.*[Mm]aritrema.*)|(.*[Ll]evinseniella.*)|(.*[Cc]urtuteria.*)|(.*[Aa]canthoparyphium.*)|
                                                           (.*[Gg]ymnophallus.*)|(.*[Aa]scarophis.*)|(.*[Aa]cuariidae.*)|(.*[Pp]rofilicollis.*)"), "parasitism", .x$type))) %>%
...
if(length(name_file) > 0){ 
  ...
  taxa_corrected <- taxa %>%
    # map(~mutate(.x, name_clear = str_remove_all(.x$original_name, "\\s\\(.*\\)$"))) %>% # Correct vernacular name at the lowest level possible (species)
    # map(~mutate(.x, name_clear = str_remove_all(.x$name_clear, "(\\s[:digit:])"))) %>%
    map(~mutate(.x, sp_corrected = str_replace_all(.x$original_name, c("colurostylis-lemu" = "Colurostylis lemurum", "diastylopsis-thil" = "Diastylopsis thileniusi",
                                                  "heterophoxus-steph" = "Heterophoxus stephenseni", "amphipod" = "Amphipoda" , "phoxocephalus-reg" = "Phoxocephalus regium",
                                                  "pontharpinia-aust" = "Pontharpinia australis", "benthic-diatoms" = "Bacillariophyta",
                                                  "exogone-hetero" = "Exogone heterosetosa", "hemigrapsus cre" = "Hemigrapsus crenulatus",
                                                  "hemigrapsus edw" = "Hemigrapsus edwardsii", "scoloplos-john" = "Scoloplos johnstonei",
                                                  "boccardia-syrtis" = "Boccardia syrtis", "boccardia-acus" = "Boccardia acus", 
                                                  "sprattus? sp." = "Sprattus sp.", "oystercatcher" = "Haematopus", "BBgull" = "Larus",
                                                  "RBgull" = "Larus", "WFheron" = "Ardea novaehollandiae", "WFtern" = "Sterna striata",
                                                  "mallard" = "Anas platyrhynchos", "hemigrasus cre" = "Hemigrapsus crenulatus", "helmsi" = "Notoacmea helmsi",
                                                  "pyramidellid" = "Pyramidellidae", "tanaidacid" = "Tanaidacidae", "paraonid" = "Paraonis",
                                                  "pelecypod" = "Pelecypoda", "spotty" = "Notolabrus celidotus"))))
  ...
  taxa_df <- taxa_checked %>%
    ...
    map2(taxa_corrected, ~left_join(.x, .y, by = c("user_supplied_name" = "sp_corrected"))) %>%
    map(~select(.x, -1)) %>%
    map2(taxa, ~right_join(.x, .y, by = c("id", "original_name"))) %>%
    map(~select(.x, original_name, matched_name)) %>%
    map(~`names<-`(.x, c("original_name", "name_clear")))
  
  taxa_df[[1]]$name_clear[6] <- "Capitellid"
  taxa_df[[1]]$name_clear[7] <- "Capitellid"
  taxa_df[[1]]$name_clear[47] <- "Pelecypoda"
  taxa_df[[1]]$name_clear[48] <- "Pelecypoda"
}
