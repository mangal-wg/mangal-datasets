...
#------------------------------
  # Metadata
#------------------------------
lat_multi_pol <- list(c(0, 0, 0),
                      conv_unit(c("-07 30 00", "-08 20 00", "-07 30 00"), "deg_min_sec", "dec_deg"))
long_multi_pol<- list(c(97, 155, 97), 
                      conv_unit(c("-80 40 00", "-81 30 00", "-80 40 00"), "deg_min_sec", "dec_deg"))
lat  <- ""
lon  <- ""
srid <- 4326

folder_name <- "vinogradov_1978" # Name of the subfolder in mangal-datasets
food_web_name <- c("WEB42")
...
ref <- list(doi       = "10.1007/bf00391640",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://doi.org/10.1007%2Fbf00391640",
             data_url  = "https://globalwebdb.com/",
             author    = "M. E. Vinogradov",
             year      = "1978",
             bibtex    = "@article{Vinogradov_1978, doi = {10.1007/bf00391640}, url = {https://doi.org/10.1007%2Fbf00391640}, year = 1978, publisher = {Springer Nature America, Inc}, volume = {48}, number = {4}, pages = {357--366}, author = {M. E. Vinogradov and E. A. Shushkina}, title = {Some development patterns of plankton communities in the upwelling areas of the Pacific Ocean}, journal = {Marine Biology}}")
...
dataset <- list(name        = "vinogradov_1978",
                 date        = "1974-01-14",
                 description = "Food web of the plankton community in the zone of upwelling",
                 public      = TRUE)
...
network <- list(name        = "vinogradov_1978",
                 date        = "1974-01-14",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
taxa_taxize <- FW_name[2] %>%
  map(~str_replace_all(.x, c("small tomopterids" = "tomopterids", "small coelenterates" = "coelenterates"))) %>%
...
taxa_df[[1]]$name_clear[8] <- "Large phytoplankton"
taxa_df[[1]]$name_clear[10] <- "Appendicularia"
taxa_df[[1]]$name_clear[11] <- "Doliolidae"
taxa_df[[1]]$name_clear[16] <- "Calanoidae"
taxa_df[[1]]$name_clear[17] <- "Tomopteridae"
taxa_df[[1]]$name_clear[18] <- "Coelenterata"
taxa_df[[1]]$name_clear[19] <- "Chaetognatha"
taxa_df[[1]]$name_clear[20] <- "Polychaeta"
...
network_id <- content(GET(url =paste0("http://poisotlab.biol.umontreal.ca/api/v2/network?dataset_id=81"))) %>% map_int(~.$id)
