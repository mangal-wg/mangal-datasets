...
refs <- list(doi       = "10.1657/1523-0430(2005)037[0514:TDAHCW]2.0.CO;2",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "http://www.bioone.org/doi/full/10.1657/1523-0430%282005%29037%5B0514%3ATDAHCW%5D2.0.CO%3B2",
             data_url  = "http://www.web-of-life.es/map.php?type=5",
             author    = "Lundgren",
             year      = "2005",
             bibtex    = "@article{article, title = {The Dense and Highly Connected World of Greenland's Plants and Their Pollinators}, author = {Lundgren, Rebekka and Olesen, Jens M.}, journal = {Arctic, Antarctic, and Alpine Research}, year = {2005}}")
...
datasets <- list(name        = "Lundgren_Olesen_2005",
                 date        = "2002-08-04",
                 description = "Pollnator activity recorded on flowers, Uummannaq Island, Greenland, Danmark",
                 public      = TRUE)


traits <- list(date = "2002-08-04")


networks <- list(name         = "Lundgen_Olesen_2005",
                 date         = "2002-08-04",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
# x  <- unname(unlist(Lundgren_Olesen_2005[1, ]))
# x[1] <- 1
# colnames(Lundgren_Olesen_2005) <- unlist(x)
# rm(x)
# 
# ## Delete unused row
# Lundgren_Olesen_2005 <- Lundgren_Olesen_2005[-1, ]
# 
# # Melt df
# Lundgren_Olesen_2005 <- melt(Lundgren_Olesen_2005, id.vars = c(1), na.rm = TRUE)
# 
# # Remove interaction value = 0 (no interaction)
# names(Lundgren_Olesen_2005) <- c("sp_taxon_1", "sp_taxon_2", "value")
# Lundgren_Olesen_2005 <- subset(Lundgren_Olesen_2005, Lundgren_Olesen_2005$value != 0)
# 
# #------------------------------
...
# taxon <- c(as.vector(unique(Lundgren_Olesen_2005$sp_taxon_2)), as.vector(unique(Lundgren_Olesen_2005$sp_taxon_1)))
# 
# 
...
# 
# for (i in 1:nrow(taxons_back_df)) {
...
POST_interactions(Lundgren_Olesen_2005, enviro = enviro, attr = attr_inter)

rm(lat, lon, srid, attr_inter, refs, users, enviro, datasets, traits, networks, inter, taxons_df, taxo_back_df, Lundgren_Olesen_2005)
