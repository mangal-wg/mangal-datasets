...
lat  <- -17.120257613156912
lon  <- 145.8645912362299
srid <- 4326
...
refs <- list(doi       = "10.1071/mf09202",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "http://www.publish.csiro.au/mf/MF09202",
            data_url  = "https://globalwebdb.com/",
            author    = "Rayner",
            year      = "2010",
            bibtex    = "@article{Rayner_2010, doi = {10.1071/mf09202}, url = {https://doi.org/10.1071%2Fmf09202}, year = 2010, publisher = {{CSIRO} Publishing}, volume = {61}, number = {8}, pages = {909}, author = {Thomas S. Rayner and Bradley J. Pusey and Richard G. Pearson and Paul C. Godfrey}, title = {Food web dynamics in an Australian Wet Tropics river}, journal = {Marine and Freshwater Research}}")
...
datasets <- list(name        = "Rayner_et_al_2010",
                date        = "2004-04-01",
                description = "Food web structure of a wet Tropic river (Mulgrave River), Australia",
                public      = TRUE)
...
networks <- list(name             = "Rayner_et_al_2010",
                date             = "2004-04-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Rayner_et_al_2010'
food_web_name <- 'WEB346'
#Read the files
...
#Modifying the interaction matrix to separate grouped species
# matrice.interaction <- rbind(matrice.interaction, matrice.interaction[6,])
# matrice.interaction[6,1] <- 'Corixidae'
# matrice.interaction[32,1] <- 'Notonectidae'
...
name.of.web[10,1] <- "Dipteran (not Chironomidae) larvae"
name.of.web[12,1] <- "Elmidae (larvae)"
name.of.web[22,1] <- "cladocera"
name.of.web[66,1] <- "Tandanus tandanus"
name.of.web[21,3] <- "Plankton"
name.of.web[31,3] <- "Plantae"
name.of.web[35,3] <- "Desmidiales"
...
matrice.interaction <- matrice.interaction[!(matrice.interaction$sp_taxon_1 == "incl. Plea"),]
matrice.interaction <- mutate(matrice.interaction, sp_taxon_1 = ifelse(sp_taxon_1 =="Cladocera etc.", "Cladocera", sp_taxon_1))
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Filamentous algae' | sp_taxon_1 == 'Aquatic macrophytes' | 
                                                                 sp_taxon_1 == 'Diatoms' | sp_taxon_1 == "Desmids", 'herbivory', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Detritus', 'commensalism', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Planktonic inverts', 'unknown', type))
...
taxons_df[21,2] <- "Melanotaenia splendida splendida"
taxons_df[54,2] <- "Algae"
taxons_df[62,2] <- "Plantae"
...
taxa_back_df[36,4] <- "101593"
taxa_back_df[37,4] <- "102160"
#------------------------------
# Set traits table
#------------------------------
