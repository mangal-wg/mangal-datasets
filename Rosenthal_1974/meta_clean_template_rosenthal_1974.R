...
folder_name <- "rosenthal_1974" # Name of the subfolder in mangal-datasets
food_web_name <- c("WEB43")

name_file <- read_csv("Trophic-metacommunities-master/Trophic_metacom_meta_analysis/Data/name_dictionary.csv",
                      col_type = cols(.default = col_character())) %>%
  filter( web %in% paste0(food_web_name)) %>%
  split(.$web)
...
dataset <- list(name        = "rosenthal_1974",
                 date        = "1967-09-01",
                 description = "Food web of the Del Mar kelp bed",
                 public      = TRUE)
...
network <- list(name        = "rosenthal_1974",
                 date        = "1967-09-01",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_2, "(.*[Pp]lant.*)|(.*[Aa]lgae.*)|(.*[Pp]hyto.*)|(.*[Dd]iatoms.*)|(.*[Dd]inofl.*)|(.*[Ss]ilicoflag.*)|
                                           (.*[Pp]roducers.*)|(.*Macrocystis.*)|(.*Pterygophora.*)"), "herbivory", "predation"))) %>% # Add type interaction
...
taxa_df <- taxa %>%
  map(~mutate(.x, name_clear = paste0(str_to_upper(str_extract(.x$original_name, ".{1}")), str_remove(.x$original_name, ".{1}")))) %>%
