...
folder_name <- "johnston_1956"
food_web_name <- c("WEB4")
...
dataset <- list(name        = "johnston_1956",
                date        = "1956-01-01",
                description = "Predation by short-eared owls on a salicornia salt marsh",
                public      = TRUE)
...
network <- list(name             = "johnston_1956",
                date             = "1956-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
# 
# ## Create taxo backbone without .sp and number
# 
# taxa_corrected <- taxa %>%
#   map(~mutate(.x, sp = str_remove_all(.x$sp, "\\s\\(.*\\)$"))) %>% # Correct vernacular name at the lowest level possible (species)
#   map(~mutate(.x, sp = str_replace_all(.x$sp, c("Sorex" = "Sorex vagrans", "passerines" = "Passeriformes", "Rattus" = "Rattus norvegicus", "heron" = "Ardeidae"))))
# 
...
# 
#   
# taxa_df[[1]]$`name_clear`[1] <- "Terrestrial plants" # Manually editing some wrong name added by Global Names Recognition and Discovery
# taxa_df[[1]]$`name_clear`[2] <- "Marine plants"
# taxa_df[[1]]$`name_clear`[6] <- "Rallus, Anas"
# taxa_df[[1]]$`name_clear`[9] <- "Migrant shorebirds and waterfowl"
# taxa_df[[1]]$`name_clear`[11] <- "Microtus, Reithrodontomys, Mus"
# taxa_df[[1]]$`name_clear`[13] <- "Circus, Asio"
