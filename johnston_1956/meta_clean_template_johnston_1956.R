...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, value = as.numeric(.$value))) %>%
...
FW_name[[1]] <- FW_name[[1]] %>%
  mutate(sp_taxon_1 = str_replace(.$sp_taxon_1, "intertidal", "intertidal invertebrates"),
         sp_taxon_2 = str_replace(.$sp_taxon_2, "intertidal", "intertidal invertebrates"))

FW_name[[2]] <- FW_name[[2]] %>%
  str_replace_all("intertidal", "intertidal invertebrates")
#------------------------------
...
taxa_df[[1]]$name_clear[9] <- "Migrant shorebirds"
...
taxa_back_df <- taxa_back_df %>%
  ...
  mutate(bold = as.double(unlist({map(.$name, ~get_boldid(.x, row = 5, verbose = FALSE)[1])})),
         ...
         ncbi = as.double(unlist({map(.$name, ~get_uid(.x, row = 5, verbose = FALSE, key = "679d0a26947d9b6432371b268ec0c7b39b08")[1])})))
