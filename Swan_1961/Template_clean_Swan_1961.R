...
lat  <- 27.886425404328126
lon  <- 87.09068298339844
srid <- 4326
...
refs <- list(doi       = "10.1038/scientificamerican1061-68",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://www.jstor.org/stable/24937104?seq=1#metadata_info_tab_contents",
             data_url  = "http://digitalcommons.rockefeller.edu/context/cohen_joel_laboratory/article/1000/type/native/viewcontent",
             author    = "Swan",
             year      = "1961",
             bibtex    = "@article{Swan_1961,	doi = {10.1038/scientificamerican1061-68}, url = {https://doi.org/10.1038%2Fscientificamerican1061-68}, year = 1961, month = {oct}, publisher = {Springer Nature}, volume = {205}, number = {4}, pages = {68--78}, author = {Lawrence W. Swan}, title = {The Ecology of the High Himalayas}, journal = {Scientific American}}")
...
datasets <- list(name        = "Swan_1961",
                 date        = "1960-10-10",
                 description = "Food web structure in the High Himalayas",
                 public      = TRUE)
...
name.of.web[c(1,3),5] <- c("Detritus","Plantae")
name.of.web <- name.of.web[-c(8,11,16,24),]
...
name.of.web[7,1] <- "Ochotona ladacensis (pika)"
name.of.web[9,1] <- "Pseudois nahura (bharal sheep)"
name.of.web[13,1] <- "Machilanus (flea)"
name.of.web[20,1] <- "Mustela altaica (weasel)"
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Flowering plants' | sp_taxon_1 == "Fungi" | sp_taxon_1 == "Pollen", 'herbivory', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Wind-blown organic debris', 'commensalism', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Dead carcasses', 'scavenger', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Pollen' & sp_taxon_2 == "Fungi", 'unknown', type))
...
taxa_back_df[2,4] <- "175945"
taxa_back_df[5,4] <- "183840"
taxa_back_df[7,4] <- "175483"
taxa_back_df[8,4] <- "562652"
