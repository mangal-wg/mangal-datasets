...
lat  <- c("29.8333333","29.8333333","29.3666667","29.3666667")
lon  <- c("118.56666666666666","119.25","119.25","118.56666666666666")
srid <- 4326
...
refs <- list(doi       = "10.1016/j.ecolmodel.2006.11.028",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://www.sciencedirect.com/science/article/pii/S030438000600603X",
            data_url  = "https://globalwebdb.com/",
            author    = "Liu",
            year      = "2007",
            bibtex    = "@article{Liu_2007,	doi = {10.1016/j.ecolmodel.2006.11.028}, url = {https://doi.org/10.1016%2Fj.ecolmodel.2006.11.028}, year = 2007, month = {may}, publisher = {Elsevier {BV}}, volume = {203}, number = {3-4}, pages = {279--289}, author = {Qi-Gen Liu and Yong Chen and Jia-Le Li and Li-Qiao Chen}, title = {The food web structure and ecosystem properties of a filter-feeding carps dominated deep reservoir ecosystem}, journal = {Ecological Modelling}}")
...
datasets <- list(name        = "Liu_et_al_2007",
                date        = "2004-01-01",
                description = "Biotic interactions and diet composition based on an Ecopath model in Lake Qiandaohu, Chun'an County, Zhejiang Province, China",
                public      = TRUE)
...
networks <- list(name             = "Liu_et_al_2007",
                date             = "2004-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
inter <- list(date          = "2004-01-01",
              direction     = "directed",
              method        = "Field observation, laboratory analysis and model",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
library(reshape2)
library(tibble)
foldername <- 'Liu_et_al_2007'
food_web_name <- 'WEB269'
#Read the files
...
name.of.web[11,2] <- "Phytoplankton"
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Phyoplankton' | sp_taxon_1 == "Benthic producer", 'herbivory', type))
...
taxons_df[16,2] <- "Elopichthys bambusa"
taxons_df[2,2] <- "Pseudolaubuca sinensis"
