...
network <- list(`Costal lagoons` = list(name             = "Costal_lagoons_yanez_1978",
                                        date             = "1973-09-01",
                                        lat              = lat,
                                        lon              = lon,
                                        srid             = srid,
              ...
              `Brackish lagoons` = list(name             = "Brackish_lagoons_yanez_1978",
                                        date             = "1973-09-01",
                                        lat              = lat,
                                        lon              = lon,
                                        srid             = srid,
...
taxa_taxize <- taxa_taxize %>% # Putting all taxize results together
  ...
  flatten_chr() %>%
  .[!duplicated(.)]
taxa_taxize["micro-"] <-"Micro-benthos"
taxa_taxize["insects"] <- "Insecta"
...
taxa_df[[1]]$name_clear[8] <- "Salmo trutta"
taxa_df[[1]]$name_clear[9] <- "Pisces"
taxa_df[[1]]$name_clear[10] <- "Salvelinus alpinus"

taxa_df[[2]]$name_clear[6] <- "Salmo trutta"
taxa_df[[2]]$name_clear[7] <- "Pisces"
taxa_df[[2]]$name_clear[8] <- "Salvelinus alpinus"
...
POST_interaction(inter_df = FW_name[[1]], inter = inter[[1]], attr = attr_inter, users = users, network = network[[1]])
## Interaction
