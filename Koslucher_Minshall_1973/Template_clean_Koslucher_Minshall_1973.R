...
lat  <- c(41.95, 41.95, 42.2333333, 42.2333333)
lon  <- c(-112.76666666666667, -112.66666666666667, -112.66666666666667, -112.76666666666667)
srid <- 4326
...
refs <- list(doi       = "10.2307/3225248",
            jstor     = "https://www.jstor.org/stable/3225248",
            pmid      = "NA",
            paper_url = "https://www.jstor.org/stable/pdf/3225248.pdf?refreqid=excelsior%3Ae2741b937f74579b7875d7ade8f58f47",
            data_url  = "https://globalwebdb.com/",
            author    = "Koslucher",
            year      = "1973",
            bibtex    = "@article{Koslucher_1973, doi = {10.2307/3225248}, url = {https://doi.org/10.2307%2F3225248}, year = 1973, month = {jul}, publisher = {{JSTOR}}, volume = {92}, number = {3}, pages = {441}, author = {Dale G. Koslucher and G. Wayne Minshall}, title = {Food Habits of Some Benthic Invertebrates in a Northern Cool-Desert Stream (Deep Creek, Curlew Valley, Idaho-Utah)}, journal = {Transactions of the American Microscopical Society}}")
...
datasets <- list(name        = "Koslucher_Minshall_1973",
                date        = "1969-11-01",
                description = "Food web strucutre of a noterhen cool-desert stream (Deep Creek), Curlew Valley, Idaho-Utah",
                public      = TRUE)
...
networks <- list(name             = "Koslucher_Minshall_1973",
                date             = "1969-11-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Koslucher_Minshall_1973'
food_web_name <- 'WEB207'
#Read the files
...
name.of.web[10,2] <- "Diatoma"
name.of.web <- name.of.web[-c(28),]
name.of.web <- name.of.web[-c(4),]
...
name.of.web$scientific_name <- str_replace_all(name.of.web$scientific_name, " sp.", "")
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Cladophora glomerata' | sp_taxon_1 == "Diatoma sp." | 
                                                                 sp_taxon_1 == "diatoms (unspec.)" | sp_taxon_1 == "Achnanthes sp." | 
                                                                 sp_taxon_1 == "Caloneis sp." | sp_taxon_1 == "Cocconeis sp." | sp_taxon_1 == "Epithemia sp." | 
                                                                 sp_taxon_1 == "Gomphonema sp." | sp_taxon_1 == "Gyrosigma sp." | sp_taxon_1 == "Meridion sp." |
                                                                 sp_taxon_1 == "Navicula sp." | sp_taxon_1 == "Nitzschia sp." | sp_taxon_1 == "Rhoicosphenia sp." | 
                                                                 sp_taxon_1 == "Rhopalodia sp." | sp_taxon_1 == "Surirella sp." | sp_taxon_1 == "Synedra sp.", 'herbivory', 'predation'))
...
taxons_df[3,2] <- "Chironomidae"
taxons_df[26,2] <- "Diatoms"
