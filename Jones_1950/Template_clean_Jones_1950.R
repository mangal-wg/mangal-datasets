...
lat  <- 52.38843631854952
lon  <- -3.8914230548195974
srid <- 4326
...
refs <- list(doi       = "10.2307/1525",
            jstor     = "https://www.jstor.org/stable/1525",
            pmid      = "NA",
            paper_url = "https://www.jstor.org/stable/1525?seq=1#metadata_info_tab_contents",
            data_url  = "https://globalwebdb.com/",
            author    = "Jones",
            year      = "1950",
            bibtex    = "@article{Jones_1950,	doi = {10.2307/1525}, url = {https://doi.org/10.2307%2F1525}, year = 1950, month = {nov}, publisher = {{JSTOR}}, volume = {19}, number = {2}, pages = {159}, author = {J. R. Erichsen Jones}, title = {A Further Ecological Study of the River Rheidol: The Food of the Common Insects of the Main-Stream}, journal = {The Journal of Animal Ecology}}")
...
datasets <- list(name        = "Jones_1950",
                date        = "1948-02-01",
                description = "Food web structure of the River Rheidol, Wales, Great Britain",
                public      = TRUE)
...
name.of.web <- name.of.web[-c(21),]
name.of.web <- select(name.of.web, split_name, scientific_name)
name.of.web[c(1,2,8),2] <- c("Plantae","Plantae","Algae")
...
matrice.interaction <- mutate(matrice.interaction, sp_taxon_1 = ifelse(sp_taxon_1 == "Dystiscidae (Deronectes", "Deronectes", sp_taxon_1))
matrice.interaction <- mutate(matrice.interaction, sp_taxon_2 = ifelse(sp_taxon_2 == "Dystiscidae (Deronectes", "Deronectes", sp_taxon_2))
matrice.interaction <- mutate(matrice.interaction, sp_taxon_1 = ifelse(sp_taxon_1 == "Oreonectes)", "Oreonectes", sp_taxon_1))
matrice.interaction <- mutate(matrice.interaction, sp_taxon_2 = ifelse(sp_taxon_2 == "Oreonectes)", "Oreonectes", sp_taxon_2))

#Adding the different types of interactions
matrice.interaction[['type']] <- NA
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Fragmented leaf' | sp_taxon_1 == "Stem tissue" | 
                                                                 sp_taxon_1 == "Moss" | sp_taxon_1 == "Diatoms" | sp_taxon_1 == "Batrachospermum" | 
                                                                 sp_taxon_1 == "Lemanea" | sp_taxon_1 == "Green algae", 'herbivory', 'predation'))
...
taxa_back_df[2,4] <- "11679"
 taxa_back_df[6,4] <- "102958"
