...
refs <- list(doi       = "NA",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://www.repository.naturalis.nl/record/504945",
            data_url  = "https://globalwebdb.com/",
            author    = "Glynn",
            year      = "1965",
            bibtex    = "@article{glynn1965community, title={Community composition, structure, and interrelationships in the marine intertidal Endocladia muricata--Balanus glandula association in Monterey Bay, California}, author={Glynn, Peter W}, journal={Beaufortia}, volume={12}, number={148}, pages={1--198}, year={1965}}")
...
datasets <- list(name        = "Glynn_1965",
                date        = "1959-05-01",
                description = "Food web structure of an intertidal community in Monterey Bay,, Pacific Grove, California",
                public      = TRUE)
...
library(reshape2)
library(tibble)
foldername <- 'Glynn_1965'
food_web_name <- 'WEB106'
#Read the files
...
name.of.web[7,2] <- "phytoplankton"
name.of.web[13,1] <- "balanus glandula"
name.of.web <- name.of.web[-c(14),]
name.of.web <- name.of.web[-c(30),]
name.of.web[29,1] <- "Pachygrapsus crassipes (juv.)"
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Diatoms' | sp_taxon_1 == "Blue-green algae" | 
                                                                 sp_taxon_1 == "Other phytoplankton", 'herbivory', 'predation'))
...
taxons_df[43,2] <- "Arenaria melanocephala"
...
taxa_back_df[7,4] <- "70401"
taxa_back_df[32,4] <- "82828"
