...
lat  <- 61.4230980962055
lon  <- 8.86362606692012
srid <- 4326
...
refs <- list(doi       = "NA",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://onlinelibrary.wiley.com/doi/abs/10.1111/j.1600-0587.1978.tb00965.x",
            data_url  = "https://globalwebdb.com/",
            author    = "Larsson",
            year      = "1978",
            bibtex    = "@article{larsson1978lake, title={The lake ecosystem of Ovre Heimdalsvatn}, author={Larsson, Petter and Brittain, John E and Lien, Leif and Lillehammer, Albert and Tangen, Karl}, journal={Ecography}, volume={1}, number={2-3}, pages={304--320}, year={1978}, publisher={Wiley Online Library}}")
...
datasets <- list(name        = "Larsson_et_al_1978",
                date        = "1969-01-01",
                description = "Food web structure of the subalpine lake Ovre Heimdalsvatn, Norway",
                public      = TRUE)
...
networks <- list(name             = "Larsson_et_al_1978",
                date             = "1969-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Larsson_et_al_1978'
food_web_name <- 'WEB126'
#Read the files
...
name.of.web[c(6,8,13,15,24,29,35,37),1] <- c("epiphytic algae","cyclops (copepod)","Leptophlebia (mayfly)","Lepidurus (tadpole shrimp)","Gammarus (amphipod)","Salmo (brown trout)","Homo (man)","Mustela (mink)")
name.of.web <- name.of.web[-c(9,14,16,25,30,36,38),]
...
matrice.interaction <- mutate(matrice.interaction, sp_taxon_1 = ifelse(sp_taxon_1 == "Epiphytic", "Epiphytic algae", sp_taxon_1))

#Adding the different types of interactions
matrice.interaction[['type']] <- NA
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Phytoplankton' | sp_taxon_1 == "Fungi" | 
                                                                 sp_taxon_1 == "Epiphytic algae" | sp_taxon_1 == "Benthic algae" | sp_taxon_1 == "Macrophytes", 'herbivory', 'predation'))
...
taxa_back_df[7,4] <- "976647"
taxa_back_df[11,4] <- "103094"
