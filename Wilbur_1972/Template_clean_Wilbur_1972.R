...
lat  <- 42.454808642193385
lon  <- -83.99992061649465
srid <- 4326
...
refs <- list(doi       = "10.2307/1935707",
            jstor     = "https://www.jstor.org/stable/1935707",
            pmid      = "NA",
            paper_url = "https://esajournals.onlinelibrary.wiley.com/doi/abs/10.2307/1935707",
            data_url  = "https://globalwebdb.com/",
            author    = "Wilbur",
            year      = "1972",
            bibtex    = "@article{Wilbur_1972, doi = {10.2307/1935707}, url = {https://doi.org/10.2307%2F1935707}, year = 1972, month = {jan}, publisher = {Wiley}, volume = {53}, number = {1}, pages = {3--21}, author = {Henry M. Wilbur}, title = {Competition, Predation, and the Structure of the Ambystoma-Rana Sylvatica Community}, journal = {Ecology}}")
...
datasets <- list(name        = "Wilbur_1972",
                date        = "1970-01-01",
                description = "Food web structure of amphibian community within woodland ponds, Edwin S. George Reserve, Michigan",
                public      = TRUE)
...
networks <- list(name             = "Wilbur_1972",
                date             = "1970-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
name.of.web[24,1] <- "Batracobdella picta (leech)"
name.of.web[19,1] <- "a. maculatum"
name.of.web[20,1] <- "a. tremblayi"
name.of.web <- name.of.web[-c(25),]
...
listafterdepooled <- sep_pooled(matrice.interaction, sep = "(\\s-\\s|[:space:]A. tr)")
...
matrice.interaction <- mutate(matrice.interaction, sp_taxon_1 = ifelse(sp_taxon_1 == "Emblayi", "A. tremblayi", sp_taxon_1))
matrice.interaction <- mutate(matrice.interaction, sp_taxon_2 = ifelse(sp_taxon_2 == "Emblayi", "A. tremblayi", sp_taxon_2))
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Periphyton', 'unknown', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Phytoplankton', 'herbivory', type))
...
#Adding the different directions of interaction
matrice.interaction[['direction']] <- "directed"
matrice.interaction <- rbind(matrice.interaction, c("Ambystoma laterale","A. maculatum","1","competition","undirected"), c("Ambystoma laterale","A. tremblayi","1","competition","undirected"),c("Ambystoma laterale","Rana sylvatica","1","competition","undirected"),
                             c("A. maculatum","A. tremblayi","1","competition","undirected"),c("A. maculatum","Rana sylvatica","1","competition","undirected"),c("A. tremblayi","Rana sylvatica","1","competition","undirected"))
...
taxons_df[c(19,20),2] <- c("Ambystoma maculatum", "Ambystoma tremblayi")
