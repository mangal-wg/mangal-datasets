...
refs <- list(doi       = "10.2307/3565588",
             jstor     = "https://www.jstor.org/stable/3565588?seq=1#metadata_info_tab_contents",
             pmid      = "NA",
             paper_url = "https://www.jstor.org/stable/pdf/3565588.pdf?refreqid=excelsior%3Acf136887b1a0398d638d20e3300d5604",
             data_url  = "https://globalwebdb.com/",
             author    = "Warren",
             year      = "1989",
             bibtex    = "@article{Warren_1989, doi = {10.2307/3565588}, url = {https://doi.org/10.2307%2F3565588}, year = 1989, month = {jul}, publisher = {{JSTOR}}, volume = {55}, number = {3},	pages = {299}, author = {Philip H. Warren}, title = {Spatial and Temporal Variation in the Structure of a Freshwater Food Web}, journal = {Oikos}}")
...
datasets <- list(name        = "Warren_1989",
                 date        = "1986-03-12",
                 description = "Food web structure of a freshwater invertebrate community, Skipwith Common, North Yorkshire, England",
                 public      = TRUE)
...
networks <- list(name             = "Warren_1989",
                 date             = "1986-03-12",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
inter <- list(date          = "1986-03-12",
              direction     = "directed",
              method        = "Laboratory observation,gut content and biblio",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
library(reshape2)
library(tibble)
foldername <- 'Warren_1989'
food_web_name <- 'WEB351'
#Read the files
...
name.of.web$original_name <- str_replace(name.of.web$original_name, "&", "and")
#Edge format
...
#matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == sp_taxon_2, 'cannibalism', 'predation'))
...
#Modifying special character in the names, so the sending to server doesnt fail
matrice.interaction$sp_taxon_1 <- str_replace(matrice.interaction$sp_taxon_1, "&", "and")
matrice.interaction$sp_taxon_2 <- str_replace(matrice.interaction$sp_taxon_2, "&", "and")
...
taxa_back_df[2,4] <- "68444"
taxa_back_df[18,4] <- "600576"
taxa_back_df[19,4] <- "605522"
taxa_back_df[20,4] <- "102124"
taxa_back_df[22,4] <- "103578"
taxa_back_df[26,4] <- "112183"
taxa_back_df[30,4] <- "112128"
#------------------------------
# Set traits table
#------------------------------
...
taxons_df <- read.csv2(paste0("mangal-datasets-ben/DBase_de_Patrick/",foldername,"/data/",foldername,"_taxa.csv"), header = TRUE)
matrice.interaction <- read.csv2(paste0("mangal-datasets-ben/DBase_de_Patrick/",foldername,"/data/",foldername,"_inter.csv"), header = TRUE)
