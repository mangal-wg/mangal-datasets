...
refs <- list(doi       = "NA",
            jstor     = "https://www.jstor.org/stable/25009256?seq=1#metadata_info_tab_contents",
            pmid      = "NA",
            paper_url = "https://www.jstor.org/stable/pdf/25009256.pdf",
            data_url  = "http://digitalcommons.rockefeller.edu/context/cohen_joel_laboratory/article/1000/type/native/viewcontent",
            author    = "Jiron",
            year      = "1981",
            bibtex    = "@article{jiron1981insect, title={Insect succession in the decomposition of a mammal in Costa Rica}, author={Jiron, Luis Fernando and Cartin, Victor M}, journal={Journal of the New York Entomological Society}, pages={158--165}, year={1981}, publisher={JSTOR}}")
...
datasets <- list(name        = "Jiron_Cartin_1981",
                date        = "1977-01-01",
                description = "The insect community of a dog carcass in a premontane humid forest, University of Costa Rica, Costa Rica",
                public      = TRUE)
...
networks <- list(name             = "Jiron_Cartin_1981",
                date             = "1977-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Jiron_Cartin_1981'
food_web_name <- 'WEB188'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'mammal carcass', 'scavenger', 'predation'))
...
write.csv2(x = matrice.interaction, paste0(file = 'mangal-datasets-ben/DBase_de_Patrick/',foldername,'/data/',foldername,'_inter.csv'), row.names = FALSE)
