...
ref <- list(doi       = "10.1007/s12237-010-9356-4",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://doi.org/10.1007%2Fs12237-010-9356-4",
            data_url  = "https://globalwebdb.com/",
            author    = "James G. Douglass",
            year      = "2011",
            bibtex    = "@article{Douglass_2011, doi = {10.1007/s12237-010-9356-4}, url = {https://doi.org/10.1007%2Fs12237-010-9356-4}, year = 2011, month = {jan}, publisher = {Springer Nature}, volume = {34}, number = {4}, pages = {701--711}, author = {James G. Douglass and J. Emmett Duffy and Elizabeth A. Canuel}, title = {Food Web Structure in a Chesapeake Bay Eelgrass Bed as Determined through Gut Contents and 13C and 15N Isotope Analysis}, journal = {Estuaries and Coasts}}")
...
network <- list(list(name        = "douglas_2011_Seegrass_food_web_april_2005",
                     ...
                     lon              = lon,
                     srid             = srid,
                     description      = "Food web of the Chesapeake's Bay Eelgrass Bed in April 2005",
                     public           = TRUE,
                ...
                list(name        = "douglas_2011_Seegrass_food_web_may_2006",
                     ...
                     lon              = lon,
                     srid             = srid,
                     description      = "Food web of the Chesapeake's Bay Eelgrass Bed in May 2006",
                     public           = TRUE,
                     all_interactions = FALSE),
                list(name        = "douglas_2011_Seegrass_food_web_august_2006",
                     ...
                     lon              = lon,
                     srid             = srid,
                     description      = "Food web of the Chesapeake's Bay Eelgrass Bed in August 2006",
                     public           = TRUE,
                     all_interactions = FALSE))
...
data_matrice <- paste0("~/Documents/UBO/Cours/Semestre 8/Stage/Mangal/Trophic-metacommunities-master/Trophic_metacom_meta_analysis/interaction matrices/",
  ...
  set_names("Seagrass April 2005", "Seagrass May 2006", "Seagrass August 2006")
...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_1, "(.*[Pp]lant.*)|(.*[Aa]lgae.*)|(.*[Pp]hyto.*)|(.*[Dd]iatoms.*)|(.*[Dd]inofl.*)|(.*[Ss]ilicoflag.*)|
                                           ...
                                           (.*[Zz]ostera.*)|(.*[Cc]occolitho.*)|(.*[Mm]acrophyt.*)"), "herbivory", "predation"))) %>% # Add type interaction
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_1, "(.*[Dd]etritu.*)|(.*[Dd]etritu.*(?!.*))|(DOM)|(Discard)|([Dd]issolved.*organic.*)|(.*[Ss]ediment.*)")
                                ,"commensalism", .x$type))) %>%
...
taxa_df <- taxa %>%
  ...
  map(~mutate(.x, name_clear = str_replace_all(.x$name_clear, fixed("Unid. Macrophytes"), "Macrophytes")))
