...
lat  <- 52.989253046392726
lon  <- -3.481503830712427
srid <- 4326
...
refs <- list(doi       = "10.2307/1599",
             jstor     = "https://www.jstor.org/stable/1599",
             pmid      = "NA",
             paper_url = "https://www.jstor.org/stable/1599?seq=1#metadata_info_tab_contents",
             data_url  = "https://globalwebdb.com/",
             author    = "Badcock",
             year      = "1949",
             bibtex    = "@article{Badcock_1949, doi = {10.2307/1599}, url = {https://doi.org/10.2307%2F1599}, year = 1949, month = {nov}, publisher = {{JSTOR}}, volume = {18}, number = {2}, pages = {193}, author = {Ruth M. Badcock}, title = {Studies in Stream Life in Tributaries of the Welsh Dee}, journal = {The Journal of Animal Ecology}}")
...
datasets <- list(name        = "Badcock_1949",
                 date        = "1946-09-01",
                 description = "Food web structure of the benthic fauna in tributaries of the Welsh Dee, United Kingdom",
                 public      = TRUE)
...
networks <- list(name             = "Badcock_1949",
                 date             = "1946-09-01",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Badcock_1949'
food_web_name <- 'WEB213'
#Read the files
...
name.of.web[c(5,7,9),5] <- c("Diatoms","Desmidiales","Plantae")
name.of.web[c(5,15,18,23,25,40,42),1] <- c("diatoms (mostly Cocconeis)","Plectoptera (indeterm.)", "protozoa (unspecified)","Entomostraca (unspec.)","Collembola (unspec.)","Salmo salar fry (0.3-5 cm)","Phoxinus phoxinus (2.0-2.9 mm)")
name.of.web <- name.of.web[-c(6,16,19,24,26,41,43),]
...
name.of.web[19,1] <- "Adult Diptera"
name.of.web[34,2] <- "Helmidae"
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Diatoms (mostly Cocconeis)' | sp_taxon_1 == "Desmids" |
                                                                 sp_taxon_1 == "Other encrusting algae" | sp_taxon_1 == "Plant tissue" | sp_taxon_1 == "Filamentous algae", 'herbivory', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Detritus' | sp_taxon_1 == "Pulverized masses", 'commensalism', type))
