...
refs <- list(doi       = "10.1093/aesa/76.4.790",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://academic.oup.com/aesa/article-abstract/76/4/790/14054?redirectedFrom=fulltext",
            data_url  = "https://www.globalwebdb.com/",
            author    = "Schoenly",
            year      = "1983",
            bibtex    = "@article{Schoenly_1983,	doi = {10.1093/aesa/76.4.790},	url = {https://doi.org/10.1093%2Faesa%2F76.4.790},	year = 1983,	month = {jul},	publisher = {Oxford University Press ({OUP})},	volume = {76},	number = {4},	pages = {790--796},	author = {Kenneth Schoenly},	title = {Arthropods Associated with Bovine and Equine Dung in an Ungrazed Chihuahuan Desert Ecosystem},	journal = {Annals of the Entomological Society of America}}")
...
datasets <- list(name        = "Schoenly_1983",
                date        = "1981-06-01",
                description = "The arthropod community of bovine and equine dung in the Chihuahuan desert near El Paso, Texas",
                public      = TRUE)
...
networks <- list(name             = "Schoenly_1983",
                date             = "1981-06-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Schoenly_1983'
food_web_name <- 'WEB198'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'bovine or equine dung', 'commensalism', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_2 == 'Philonthus sp.' | sp_taxon_2 == 'Aleochara sp.' | sp_taxon_2 == 'Saprinus pennsylvanicus'
                                                                 | sp_taxon_2 == 'Psilochorus imitatus' | sp_taxon_2 == 'Lycosa coloradensis', 'predation', type))
...
write.csv2(x = taxa_back_df, file = "mangal-datasets-ben/DBase_de_Patrick/Schoenly_1983/data/Schoenly_1983_taxa_back.csv", row.names = FALSE)
write.csv2(x = taxons_df, file = "mangal-datasets-ben/DBase_de_Patrick/Schoenly_1983/data/Schoenly_1983_taxa.csv", row.names = FALSE)
write.csv2(x = matrice.interaction, file = "mangal-datasets-ben/DBase_de_Patrick/Schoenly_1983/data/Schoenly_1983_inter.csv", row.names = FALSE)
