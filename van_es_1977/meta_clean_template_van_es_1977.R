...
folder_name <- "van_es_1977" # Name of the subfolder in mangal-datasets
food_web_name <- "WEB121"
...
ref <- list(doi       = "10.1007/BF02207842",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://doi.org/10.1007/BF02207842",
             data_url  = "https://globalwebdb.com/",
             author    = "F. B. van Es",
             year      = "1977",
             bibtex    = "@article{Es_1977, doi = {10.1007/bf02207842}, url = {https://doi.org/10.1007%2Fbf02207842}, year = 1977, month = {aug}, publisher = {Springer Nature}, volume = {30}, number = {1-4}, pages = {283--294}, author = {F. B. Es}, title = {A preliminary carbon budget for a part of the Ems estuary: The Dollard}, journal = {Helgoländer Wissenschaftliche Meeresuntersuchungen}}")
...
dataset <- list(name        = "van_es_1967",
                 date        = "1965-01-01",
                 description = "Food web of the Ems estuary",
                 public      = TRUE)
...
#   select(2) %>% # Only the second column got the species names.
...
#   map(~filter(.x, X2 != is.na(X2))) %>% # Remove the last line contains only NA
...
FW_name <- data_matrice %>% # Construct the Food Web matric to inject
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_1, "(.*[Pp]lant.*)|(.*[Aa]lgae.*)|(.*[Pp]hyto.*)|(.*[Dd]iatoms.*)|(.*[Dd]inofl.*)")
