...
lat  <- 40.09186086653505
lon  <- -88.23071715366854
srid <- 4326
...
refs <- list(doi       = "10.2307/1943223",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://esajournals.onlinelibrary.wiley.com/doi/abs/10.2307/1943223",
            data_url  = "https://globalwebdb.com/",
            author    = "Mohr",
            year      = "1943",
            bibtex    = "@article{Mohr_1943,	doi = {10.2307/1943223},  url = {https://doi.org/10.2307%2F1943223},	year = 1943,	month = {feb},	publisher = {Wiley},	volume = {13},	number = {3},	pages = {275--298},	author = {Carl O. Mohr},	title = {Cattle Droppings as Ecological Units},	journal = {Ecological Monographs}}")
...
datasets <- list(name        = "Mohr_1943",
                date        = "1930-10-01",
                description = "Insect community in cattle droppings in unwooded pastures at Urbana, Illinois",
                public      = TRUE)
...
library(reshape2)
library(tibble)
foldername <- 'Mohr_1943'
food_web_name <- 'WEB197'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'dung', 'commensalism', NA))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_2 == 'Sphaeridium scarabaeoides' | sp_taxon_2 == 'Platystethus americanus' |
                                                                   sp_taxon_2 == 'other staphylinids (unspec.)' | sp_taxon_2 == 'Sphaeridium bipustulatum' | sp_taxon_2 == 'Hister abbreviatus', 'predation', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_2 == 'Xyalophora quinquelineata' | sp_taxon_2 == 'Phaneropria sp.', 'parasitism', type))
...
write.csv2(x = taxa_back_df, file = "mangal-datasets-ben/DBase_de_Patrick/Mohr_1943/data/Mohr_1943_taxa_back.csv", row.names = FALSE)
write.csv2(x = taxons_df, file = "mangal-datasets-ben/DBase_de_Patrick/Mohr_1943/data/Mohr_1943_taxa.csv", row.names = FALSE)
write.csv2(x = matrice.interaction, file = paste0("mangal-datasets-ben/DBase_de_Patrick/Mohr_1943/data/Mohr_1943_inter.csv"), row.names = FALSE)
