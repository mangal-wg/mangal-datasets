...
refs <- list(doi       = "10.1017/S0080455X00012546",
            jstor     = "NA",
            pmid      = "NA",
            paper_url = "https://www.cambridge.org/core/journals/proceedings-of-the-royal-society-of-edinburgh-section-b-biological-sciences/article/summary-of-the-loch-leven-ibp-results-in-relation-to-lake-management-and-future-research/29C44AFB6577A99956647C53FE1F8111",
            data_url  = "https://globalwebdb.com/",
            author    = "Morgan",
            year      = "1974",
            bibtex    = "@article{morgan1974summary, doi={10.1017/S0080455X00012546}, title={A summary of the Loch Leven IBP results in relation to lake management and future research}, author={Morgan, NC and McLusky, DS}, journal={Proceedings of the Royal Society of Edinburgh, Section B: Biological Sciences}, volume={74}, pages={407--416}, year={1974}, publisher={Royal Society of Edinburgh Scotland Foundation}}")
...
datasets <- list(name        = "Morgan_McLusky_1972",
                date        = "1971-01-01",
                description = "The biotic interactions within Loch Leven, Kinross, United Kingdom",
                public      = TRUE)
...
networks <- list(name             = "Morgan_McLusky_1972",
                date             = "1971-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Morgan_McLusky_1972'
food_web_name <- 'WEB68'
#Read the files
...
name.of.web[9,2] <- "Plantae"
name.of.web[24,2] <- "Insectae"
...
# matrice.interaction <- rbind(matrice.interaction, matrice.interaction[5,])
# matrice.interaction[5,1] <- 'Phragmites'
# matrice.interaction[15,1] <- 'Potamogeton'
# matrice.interaction <- cbind(matrice.interaction, matrice.interaction[,8], matrice.interaction[,8], stringsAsFactors = F)
# matrice.interaction[1,8] <- 'pochard'
# matrice.interaction[1,20] <- 'swan'
# matrice.interaction[1,21] <- 'coot'
# matrice.interaction <- cbind(matrice.interaction, matrice.interaction[,11], stringsAsFactors = F)
# matrice.interaction[1,11] <- 'nematodes'
# matrice.interaction[1,22] <- 'ciliates'
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'benthic algae' | sp_taxon_1 == "phytoplankton" |
                                                                 sp_taxon_1 == "Phragmites" | sp_taxon_1 == "Potamogeton" | 
                                                                 sp_taxon_1 == "herb", 'herbivory', type))
matrice.interaction <- matrice.interaction[-c(6,8),]
