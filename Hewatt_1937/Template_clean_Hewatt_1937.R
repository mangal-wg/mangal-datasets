...
refs <- list(doi       = "10.2307/2420496",
            jstor     = " https://www.jstor.org/stable/2420496",
            pmid      = "NA",
            paper_url = "https://www.jstor.org/stable/2420496?seq=1#metadata_info_tab_contents",
            data_url  = "https://globalwebdb.com/",
            author    = "Hewatt",
            year      = "1937",
            bibtex    = "@article{Hewatt_1937, doi = {10.2307/2420496}, url = {https://doi.org/10.2307%2F2420496},	year = 1937,	month = {mar},	publisher = {{JSTOR}},	volume = {18},	number = {2},	pages = {161},	author = {Willis G. Hewatt},	title = {Ecological Studies on Selected Marine Intertidal Communities of Monterey Bay, California},	journal = {American Midland Naturalist}}")
...
datasets <- list(name        = "Hewatt_1937",
                date        = "1931-10-01",
                description = "Food web structure of intertidal communities, Monterey Bay, Cabrillo point, California",
                public      = TRUE)
...
networks <- list(name             = "Hewatt_1937",
                date             = "1931-10-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
                description      = "Food web structure of a protected rocky intertidal community, Monterey Bay, Cabrillo point, California",
                public           = TRUE,
                all_interactions = FALSE)


inter <- list(date          = "1931-10-01",
              direction     = "directed",
...
library(reshape2)
library(tibble)
foldername <- 'Hewatt_1937'
food_web_name <- 'WEB108'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Detritus' | sp_taxon_1 == "Suspended detritus", 'commensalism', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Phytoplankton' | sp_taxon_1 == "Encrusting and mat-forming algae" |
                                                                 sp_taxon_1 == "Seaweeds", 'herbivory', type))
