...
lat  <- 13.273156397678223
lon  <- -59.64702912425264
srid <- 4326
...
datasets <- list(name        = "Briand_1985",
                #date        = "1111-11-11",
                description = "Food web structure of a western rocky shore, Barbados (Unpublished observations)",
                public      = TRUE)
...
networks <- list(name             = "Briand_1985",
                #date             = "1111-11-11",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Organic debris' | sp_taxon_1 == "Detritus", 'commensalism', 'predation'))
