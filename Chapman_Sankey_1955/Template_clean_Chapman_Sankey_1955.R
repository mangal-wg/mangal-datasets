...
lat  <- 51.26143146485018
lon  <- -0.32100677490234375
srid <- 4326
...
refs <- list(doi       = "10.2307/1720",
             jstor     = "https://www.jstor.org/stable/1720",
             pmid      = "NA",
             paper_url = "https://www.jstor.org/stable/1720?seq=1#metadata_info_tab_contents",
             data_url  = "http://digitalcommons.rockefeller.edu/context/cohen_joel_laboratory/article/1000/type/native/viewcontent",
             author    = "Chapman",
             year      = "1955",
             bibtex    = "@article{Chapman_1955, doi = {10.2307/1720}, url = {https://doi.org/10.2307%2F1720}, year = 1955, month = {nov}, publisher = {{JSTOR}}, volume = {24}, number = {2}, pages = {395}, author = {R. F. Chapman and J. H. P. Sankey}, title = {The Larger Invertebrate Fauna of Three Rabbit Carcasses}, journal = {The Journal of Animal Ecology}}")
...
datasets <- list(name        = "Chapman_Sankey_1955",
                 date        = "1951-07-05",
                 description = "Food web structure of rabbit carrions, Juniper Hall Field Centre, Dorking, England",
                 public      = TRUE)
...
networks <- list(name             = "Chapman_Sankey_1955",
                 date             = "1951-07-05",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Chapman_Sankey_1955'
food_web_name <- 'WEB185'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'rabbit carcass', 'scavenger', 'predation'))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_2 == 'Figites laevigatus' | sp_taxon_2 == "Aleochara curtula" |
                                                                 sp_taxon_2 == "Alysia manducator", 'parasitism', type))
...
taxons_df[27,2] <- "Liopiophila varipes"
...
taxa_back_df[11,4] <- "718773"
taxa_back_df[22,4] <- "143263"
