...
ref <- list(doi       = "10.1098/rstb.2006.1956",
             jstor     = "NA",
             pmid      = "NA",
...
dataset <- list(name        = "smith_2007",
                 date        = "2007-01-01",
                 description = "Food web of the Ross Sea continental shelf ecosystem",
                 public      = TRUE)
...
FW_name <- data_matrice %>%
  ...
  map(~mutate(.x, type = ifelse(str_detect(.x$sp_taxon_1, "(.*[Dd]ebris.*)|(.*[Dd]etritu*.(?!.*fish.*))|(DOM)"), "commensalism", .x$type))) %>%
...
# 
# sp_name <- taxa %>%
#   map(~mutate(.x, sp = str_replace_all(.x$sp, c("DOM" = "Dissolved organic matter", "Cryptophytes" = "Cryptophyta", "Diatoms" = "Bacillariophyta",
#                     "Pteropods" = "Pteropoda", "Crystal Krill" = "Euphausia crystallorophias", "Baleen whales" = "Mysticeti",
#                     "Crabeater seals" = "Lobodon carcinophaga", "Weddell seals" = "Leptonychotes weddellii", "Emperor penguins" = "Aptenodytes forsteri",
#                     "Adelie penguins" = "Pygoscelis adelia", "Killer Whales A" = "Orcinus orca", "Killer Whales B" = "Orcinus orca", 
#                     "Killer Whales C" = "Orcinus orca", "Squid" = "Pyschroteuthis glacialis", "Leopard seals" = "Hydrurga leptonyx",
