...
refs <- list(doi       = "10.1111/j.1461-0248.2012.01777.x",
             jstor     = "NA",
             pmid      = "NA",
             paper_url = "https://onlinelibrary.wiley.com/doi/full/10.1111/j.1461-0248.2012.01777.x",
             data_url  = "https://globalwebdb.com/",
             author    = "Boit",
             year      = "2012",
             bibtex    = "@article{Boit_2012,	doi = {10.1111/j.1461-0248.2012.01777.x}, url = {https://doi.org/10.1111%2Fj.1461-0248.2012.01777.x}, year = 2012, month = {apr}, publisher = {Wiley}, volume = {15}, number = {6}, pages = {594--602}, author = {Alice Boit and Neo D. Martinez and Richard J. Williams and Ursula Gaedke}, title = {Mechanistic theory and modelling of complex food-web dynamics in Lake Constance}, journal = {Ecology Letters}}")
...
datasets <- list(name        = "Boit_et_al_2012",
                 date        = "1987-01-01",
                 description = "Food web structure of Lake Constance based on an Ecopath model, Central Europe",
                 public      = TRUE)
...
networks <- list(name             = "Boit_et_al_2012",
                 date             = "1987-01-01",
                 lat              = lat,
                 lon              = lon,
                 srid             = srid,
...
inter <- list(date          = "1987-01-01",
              direction     = "directed",
              method        = "Biblio and model",
              description   = "null",
              public        = TRUE,
              lat           = lat,
              lon           = lon,
...
library(reshape2)
library(tibble)
foldername <- 'Boit_et_al_2012'
food_web_name <- 'WEB345'
#Read the files
...
name.of.web[c(18,19),1] <- c("Diatoms","algal colonies")
name.of.web <- name.of.web[!duplicated(name.of.web$original_name), ]
name.of.web[c(6,13,16,18),2] <- c("rotifera","algae","Heterotrophic nanoflagellate","ciliophora")
...
name.of.web$original_name <-  paste0(str_to_upper(str_extract(name.of.web$original_name, ".{1}")), str_remove(name.of.web$original_name, ".{1}"))
...
listafterdepooled <- sep_pooled(matrice.interaction, sep = "s - ")
...
matrice.interaction <- mutate(matrice.interaction, sp_taxon_1 = ifelse(sp_taxon_1 == "Diatom", "Diatoms", sp_taxon_1))

#Adding the different types of interactions
matrice.interaction[['type']] <- NA
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Diatoms' | sp_taxon_1 == "Filamentous blue and green algae" |
                                                                 sp_taxon_1 == "Algal colonies" | sp_taxon_1 == "Small coccal algae" | 
                                                                 sp_taxon_1 == "Autotrophic picoplankton" | sp_taxon_1 == "Single-cell algae" |
                                                                 sp_taxon_1 == "Large single-cell algae or colonies", 'herbivory', 'predation'))
