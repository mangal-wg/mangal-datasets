...
refs <- list(doi       = "NA",
            ...
            author    = "Nybakken",
            year      = "2001",
            bibtex    = "@book{nybakken2001marine, title={Marine biology: an ecological approach}, author={Nybakken, James Willard and Bertness, Mark D}, volume={5}, year={2001}, publisher={Benjamin Cummings San Francisco}")
...
datasets <- list(name        = "Nybakken_2001",
                #date        = "1111-11-11",
                description = "Food web structure of an exposed intertidal sand beach community, California",
                public      = TRUE)
...
networks <- list(name             = "Nybakken_2001",
                #date             = "1111-11-11",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'debris', 'commensalism', 'predation'))
...
taxa_back_df[2,4] <- "98127"
taxa_back_df[7,4] <- "67377"
