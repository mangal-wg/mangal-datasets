...
datasets <- list(name        = "Angelini_et_al_2013",
                date        = "1988-01-01",
                description = "Food web structure in term of diet composition of three oxbow lakes, Pantanal Plain, Paraguay River basin, Brazil",
                public      = TRUE)
...
folder.name <- 'Angelini_et_al_2013'
...
names(matrices.interaction) <- c('WEB254', 'WEB255', 'WEB256')
...
names.of.web <- list(subset(name.dictionary, name.dictionary$web == 'WEB254.csv'), subset(name.dictionary, name.dictionary$web == 'WEB255.csv'), 
                     subset(name.dictionary, name.dictionary$web == 'WEB256.csv'))
names(names.of.web) <- c('WEB254', 'WEB255', 'WEB256')
...
matrices.interaction <- map(matrices.interaction, ~mutate(.x, type = ifelse(sp_taxon_1 == 'Phytoplankton' | sp_taxon_1 == 'Macrophytes', 'herbivory', 'predation')))
matrices.interaction <- map(matrices.interaction, ~mutate(.x, type = ifelse(sp_taxon_1 == 'Detritus', 'commensalism', type)))
matrices.interaction <- map(matrices.interaction, ~mutate(.x, type = ifelse(sp_taxon_1 == 'Import', 'unknown', type)))
...
#------------------------------
meta <- read.table(file="mangal-datasets-ben/DBase_de_Patrick/Angelini_et_al_2013/raw/meta.txt", header = T, sep = "\t", stringsAsFactors = F)
for (i in 1:3) {
  
  
  networks <- list(name             = paste0('Angelini_et_al_2013_',i),
                  date             = "1988-01-01",
                  lat              = meta[i,2],
                  lon              = meta[i,3],
                  srid             = srid,
                  description      = paste0("Food web structure in term of diet composition of ",meta[i,1],", Pantanal Plain, Paraguay River basin, Brazil"),
                  public           = TRUE,
                  all_interactions = FALSE)
  
  inter <- list(date          = "1988-01-01",
                direction     = "directed",
                method        = "Gut content and model",
                description   = "null",
                public        = TRUE,
                lat           = meta[i,2],
                lon           = meta[i,3],
