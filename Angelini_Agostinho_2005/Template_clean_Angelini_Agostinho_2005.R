...
datasets <- list(name        = "Angelini_Agostinho_2005",
                date        = "1992-01-01",
                description = "Food web structure in term of diet composition in the Parana River, Brazil",
                public      = TRUE)
...
networks <- list(name             = "Angelini_Agostinho_2005",
                date             = "1992-01-01",
                lat              = lat,
                lon              = lon,
                srid             = srid,
...
library(reshape2)
library(tibble)
foldername <- 'Angelini_Agostinho_2005'
food_web_name <- 'WEB249'
#Read the files
...
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Periphyton' | sp_taxon_1 == 'Benthos', 'unknown', type))
matrice.interaction <- mutate(matrice.interaction, type = ifelse(sp_taxon_1 == 'Phytoplankton' | sp_taxon_1 == "Aquatic macrophytes", 'herbivory', type))
...
taxons_df[30,2] <- "Brycon orbignyanus"
taxons_df[23,2] <- "Acestrorhynchus lacustris"
